...
class Solution {
    ...
    isValidBST(root) {
        ...
        let rootCheck = this.isRootValid(root);
    ...
    isRootValid(root){
        ...
        if(!root.left && root?.right?.val > root.val) return true; 
        if(!root.right && root.left?.val < root.val) return true; 
